...
const CategorySelector: React.FC<CategorySelectorProps> = ({
  ...
  return (
    <div className="flex justify-center mb-8 overflow-x-auto pb-2">
      <div className="inline-flex bg-slate-900/50 backdrop-blur-xl p-2 rounded-2xl border border-white/10 shadow-xl">
        {categories.map((category) => (
          <button
            key={category.id}
            onClick={() => onCategoryChange(category.id)}
            className={`relative px-6 py-4 text-sm md:text-base font-semibold rounded-xl transition-all duration-300 whitespace-nowrap overflow-hidden group
